...
function calculatePrizePoolContribution(amountEtb: number): number {
...
export function getConfiguredEntryFeeEtb(): number {
  if (!Number.isSafeInteger(ENTRY_FEE_ETB) || ENTRY_FEE_ETB < 0) {
    return 0;
  }
...
export async function ensureGwCompetition(
  ...
  const fee = PAID_COMPETITION_ENABLED
  ...
  if (existing[0]) {
    if (
      ...
      const updated = await db
        ...
        .where(
          and(
            eq(gwCompetitions.id, existing[0].id),
  ...
  if (!inserted[0]) {
    throw new Error("Could not create GW competition");
  }
...
function makeTxRef(
  ...
  return `fpl-${competitionId.replace(
    /[^a-z0-9-]/gi,
    "-",
  )}-${userId}-${crypto.randomBytes(5).toString("hex")}`.slice(
    0,
    100,
  );
}
  ...
  return db.transaction(async (tx) => {
    ...
    const saved = await tx
      .select({
        ...
        checkoutUrl: gwPayments.checkoutUrl,
      })
      .from(gwPayments)
      .where(
        and(
...
export async function confirmPaymentFromProvider(
  ...
  const payment = await db
    .select()
    .from(gwPayments)
    .where(eq(gwPayments.txRef, txRef))
    .limit(1)
    .then((rows) => rows[0]);
